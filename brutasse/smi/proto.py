...
class TlvRemote(Tlv, type_id=3):
    ...
    def build(self) -> bytes:
        return struct.pack('!336s336s336s', self.cmd1.encode(), self.cmd2.encode(), self.cmd3.encode())

    @classmethod
    def parse(cls, raw: bytes) -> Self:
        cmd1, cmd2, cmd3 = struct.unpack('!336s336s336s', raw)
        return cls(cmd1.rsplit(b'\0').decode(), cmd2.rsplit(b'\0').decode(), cmd3.rsplit(b'\0').decode())
